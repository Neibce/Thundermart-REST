...
function checkExist(providerId, provider, callback) {
    pool.query('SELECT COUNT(*) FROM T_USER_LIST WHERE USER_PROVIDER_ID=? AND PROVIDER=?;', [providerId, provider],
        function(err, data){
            if (err){
                console.log(err);
                return callback(3);
...
function create(providerId, provider, nickname, clientIp, callback) {
    checkExist(providerId, provider,
        function(resCode, exist) {
            if(resCode != 0)
                return callback(resCode);
            if(!exist){
                pool.query('INSERT INTO T_USER_LIST(USER_ID, USER_CREATE_IP, USER_NICKNAME, USER_PROVIDER_ID, PROVIDER) VALUES (UNHEX(REPLACE(UUID(),\'-\',\'\')), ?, ?, ?, ?);',
                    [clientIp, nickname, providerId, provider],
                    function(err, data){
                        if (err){
                            console.log(err);
                            return callback(3);
